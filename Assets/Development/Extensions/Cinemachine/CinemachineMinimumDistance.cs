...
{
    /// <summary>
    /// An add-on module for Cinemachine Virtual Camera prevents the camera from going closer than a minimum distance from it's target.
    /// </summary>
    [ExecuteInEditMode]
    ...
    public class CinemachineMinimumDistance : CinemachineExtension
    {
        ...
        void OnValidate()
        {
            m_ActualMinimumDistanceFromTarget = Mathf.Max(0.01f, m_ActualMinimumDistanceFromTarget);
        }
    ...
    public class CinemachineColliderPlus : CinemachineExtension
    {
        ...
        /// <summary>Obstacles closer to the target than this will be ignored.</summary>
        [Tooltip("Ostacles closer to the target than this will be ignored")]
        public float m_ExcludeDistanceFromTarget = 0.01f;
        ...
        void OnValidate()
        {
            ...
            m_ExcludeDistanceFromTarget = Mathf.Max(0.01f, m_ExcludeDistanceFromTarget);
        ...
        Vector3 RespectCameraRadius(Vector3 cameraPos, Vector3 lookAtPos)
        {
            ...
            if (numObstacles == 0 && m_TransparentLayers != 0
                && distance > m_ExcludeDistanceFromTarget + Epsilon)
            {
            ...
            if (numObstacles > 0 && distance == 0 || distance > m_ExcludeDistanceFromTarget)
            {
        ...
        bool CheckForTargetObstructions(CameraState state)
        {
            if (state.HasLookAt)
            {
                ...
                if (RuntimeUtility.RaycastIgnoreTag(ray, out _,
                        distance - m_ExcludeDistanceFromTarget,
                        m_CollideAgainst & ~m_TransparentLayers, m_IgnoreTag))
                    return true;
            }
            return false;
