//SEE: https://github.com/Unity-Technologies/Graphics/blob/v10.10.1/com.unity.render-pipelines.high-definition/Editor/Material/UIBlocks/EmissionUIBlock.cs
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        else
                        {
                            ...
                            if (!intensityIsMixed)
                            {
                                ...
                                float evValue = ConvertLuminanceToEv(emissiveIntensity.floatValue);
                                evValue = EditorGUILayout.FloatField(Styles.emissiveIntensityText, evValue);
                                newIntensity = Mathf.Clamp(evValue, 0, s_MaxEvValue);
                                emissiveIntensity.floatValue = ConvertEvToLuminance(evValue);
                            }
                            else
                            {
                                using (var change = new EditorGUI.ChangeCheckScope())
